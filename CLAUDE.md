...
- Authoring traps the 2026-09-03 REFUTE passes caught repeatedly, check every new entry against them: entering an EMPTY enemy battlefield is not an attack (Assault and "When I attack" need the Attacker designation, which exists only inside a Combat with a garrison: 807.1.d, 383.4.e, 461); Energy added in Awaken or the Beginning Phase is lost when the Rune Pool empties at the Main Phase start (167); a Repeat gives no window to re-exhaust a resource permanent between executions (429.3, R21); Temporary tokens die before Scoring, so they never Hold and never see a Main Phase start (816.1.b); Gold tokens are printed "exhausted", so whether Renata Industrialist readies them is open (R25); recycling a rune for Power sends it to the Rune Deck (161.2.b), so Ekko readies only what is still on the board; a recall is not a move (456) and does not change state (458).
